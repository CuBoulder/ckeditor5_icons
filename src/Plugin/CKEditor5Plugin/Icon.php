...
class Icon extends CKEditor5PluginDefault implements ContainerFactoryPluginInterface {
	...
	public function __construct(array $configuration, $plugin_id, $plugin_definition, ExtensionPathResolver $extensionPathResolver) {
		...
		$this->faCategories = Yaml::parseFile($modulePath . '/libraries/fontawesome6/metadata/categories.yml');
		$this->faIcons = array_map(function($item) {
			return [
				'styles' => $item['styles'],
				'label' => $item['label']
			];
		}, Yaml::parseFile($modulePath . '/libraries/fontawesome6/metadata/icons.yml'));
	}
